            ...
            if(!(dir[0xB] & 8)) {
                ...
                if(rddir && rd) {
                    ...
                    if(c >= 2 && c < 0x0FFFFFF8 && plgs + 512 < plgids + 0x40000) {
                        /* we must do this to avoid misaligned data abort with plgs */
                        fw_loadsec(c * bpb->spc + data_lba, (void*)file_buf);
                        memcpy(plgs, (void*)file_buf, 512);
        ...
        if(plgs[10] == type) {
            ...
            if(type != PLG_T_TAG) {
                ...
                for(i = 0; i < plgs[11]; i++, ids++) {
                    ...
                    switch(ids->type) {
                        ...
                        case PLG_M_WORD: n = o; o = 0; if(n <= 65534) memcpy(&o, buf + n, 2); break;
                        case PLG_M_DWORD:n = o; o = 0; if(n <= 65532) memcpy(&o, buf + n, 4); break;
                        case PLG_M_BADD: o = (o > 65535 ? 0 : buf[o]) + offs; break;
                        case PLG_M_WADD: n = o; o = 0; if(n <= 65534) { memcpy(&o, buf + n, 2); } o += offs; break;
                        case PLG_M_DADD: n = o; o = 0; if(n <= 65532) { memcpy(&o, buf + n, 4); } o += offs; break;
                    }
                    if(!ids->size) offs = o;
            ...
            if(!a) {
                ...
                if(fw_open(fn)) {
                    ...
                    a = (uint64_t)alloc((hdr->memsz + 4095) >> 12);
                    memcpy(plgs + 2, &a, 8);
                    hdr = (plg_hdr_t *)a;
                    ...
                    for(i = 0, r = (plg_rel_t*)((uint8_t*)hdr + sizeof(plg_hdr_t) + hdr->nid * sizeof(plg_id_t));
                      i < hdr->nrel; i++, r++) {
                        ...
                        for(m = 1, n = PLG_R_SBIT(rel.type); m && n <= PLG_R_EBIT(rel.type); m <<= 1)
                            if(t & m) {
                                if(b & m) imm |= (uint64_t)1UL << (uint64_t)n;
                                n++;
                            }
                        /* sign extend */
                        ...
                        if(n) {
                            if(imm >> 63UL) { imm = -imm; b |= (uint64_t)1UL << (uint64_t)n; }
                            else b &= ~((uint64_t)1UL << (uint64_t)n);
                        }
                        /* write back new immedate */
                        for(m = 1, n = PLG_R_SBIT(rel.type); m && n <= PLG_R_EBIT(rel.type); m <<= 1)
                            if(t & m) {
                                if(imm & ((uint64_t)1UL << (uint64_t)n)) b |= m;
                                else b &= ~m;
                                n++;
                            }
                        memcpy(addr, &b, 8);
            ...
            if(type == PLG_T_TAG) (*((void(* PLG_API)(void))((uintptr_t)((plg_hdr_t *)a)->entry)))();
            else return (plg_hdr_t *)a;
        }
    return NULL;
}
...
err:    smp = 0; return 0;
    ...
    if(smp) {
        ...
        *((volatile uint64_t*)0x538) = (uint64_t)(uintptr_t)ent;
