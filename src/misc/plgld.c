...
struct { uint16_t arch; uint32_t type; uint8_t calc; uint8_t sbit; uint8_t ebit; uint8_t imsk; uint8_t neg; } rel_types[] = {
    ...
    { EM_AARCH64,   R_AARCH64_ADR_GOT_PAGE,         REL_GAP,    0, 11, 4, 0 },
    { EM_AARCH64,   R_AARCH64_LD64_GOT_LO12_NC,     REL_GAP,    0, 11, 5, 0 }
};
...
uint64_t imasks[6] = { PLG_IM_0, PLG_IM_AARCH64_1, PLG_IM_AARCH64_2, PLG_IM_AARCH64_3, PLG_IM_AARCH64_4, PLG_IM_AARCH64_5 };
...
{
    char *str;
    uint32_t i, j, k, l, n, *ptr, code, hdrsz, bssndx = 0;
...
usage:  printf("Easyboot Plugin Linker, Copyright (c) 2023 bzt, GPLv3+\r\nhttps://codeberg.org/bzt/easyboot\r\n\r\n");
    ...
    for(i = n = 0; i < ehdr->e_shnum; i++) {
        ...
        if((s->sh_type == SHT_NOBITS && s->sh_flags == (SHF_WRITE | SHF_ALLOC)) ||
          /* workaround a Clang bug, it supposedly generates OS-specific sections in freestanding mode... */
          (s->sh_type >= SHT_LOOS && !bssndx)) {
            s->sh_link = SHT_BSS;
            bssndx = i;
        } else
    ...
    for(i = 0; i < ehdr->e_shnum; i++) {
        ...
        for(j = 0; j < s->sh_size / s->sh_entsize; j++) {
            ...
            /* do some extra checks and workaround a Clang bug with OS-specific sections in freestanding mode */
            if(sym->st_shndx == SHN_COMMON && bssndx) sym->st_shndx = bssndx;
            if(sym->st_shndx >= ehdr->e_shnum) {
                if(sym->st_shndx >= 0xff00)
                    fprintf(stderr, "plgld: compiler bug, OS-specific section %x for symbol #%u '%s', shouldn't happen in freestanding mode\r\n",
                        sym->st_shndx, ELF64_R_SYM(rel->r_info), str);
                else
                    fprintf(stderr, "plgld: invalid st_shndx %u (max %u) for symbol #%u '%s'\r\n",
                        sym->st_shndx, ehdr->e_shnum, ELF64_R_SYM(rel->r_info), str);
