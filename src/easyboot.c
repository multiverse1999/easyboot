...
void etbc_create(void)
{
    ...
    memcpy(&iso[318], "EASYBOOT <HTTPS://CODEBERG.ORG/BZT/EASYBOOT>", 44);
    memcpy(&iso[446], "EASYBOOT", 8);
    memcpy(&iso[574], "BOOT CDROM", 9);
...
int dev_write(void *f, uint64_t off, void *buf, uint32_t size)
{
    ...
    if(r < size) {
        /* issue #7: Win reports an 0x57 ERROR_INVALID_PARAMETER if size isn't multiple of 512 */
 ...
 */
int check_magic(uint8_t *buf, uint8_t *plg, uint32_t len)
{
    ...
    for(i = 0; i < hdr->nid; i++, ids++) {
        ...
        if(!ids->size) offs = o;
            ...
            if(o + ids->size >= len) return 0;
        } else if(o + ids->size >= len || memcmp(buf + o, ids->magic, ids->size)) return 0;
    }
    return 1;
}
...
void check_partitions(void)
{
    ...
    for(j = 0; j < 9; j++)
        if(memcmp(&menupart[j], &empty, sizeof(guid_t))) {
            ...
            for(i = 0; i < num_plgs && !check_magic(buf, plgs[i], 65536) && !check_magic(buf + 65536, plgs[i], 65536); i++);
...
int main(int argc, char **argv)
{
    ...
    /* max size comes from boot_x86.asm line 72 times 2, sectop is the same plus the bss size */
    if(sizeof(loader_x86_efi) > 120 * 2 * 512 || !sectop || sectop >= 0x20000 || !j) {
    ...
    if(!(img = (uint8_t*)malloc((GPT_SECTORS + 2) << 9))) { fprintf(stderr, "easyboot: %s\r\n", lang[ERRMEM]); goto err; }
    ...
    if(!memcmp(img + 512, EFI_PTAB_HEADER_ID, 8)) {
        ...
        if(!dev_read(f, ((gpt_header_t*)(img + 512))->PartitionEntryLBA << 9, img + 1024, GPT_SECTORS * 512)) goto noesp;
        for(i = 0; i + j <= GPT_SECTORS * 512; i += j)
            if(!memcmp(&((gpt_entry_t*)(img + 1024 + i))->PartitionTypeGUID, &espGuid, sizeof(guid_t))) {
                boot_lba = ((gpt_entry_t*)(img + 1024 + i))->StartingLBA;
                partsize = (((gpt_entry_t*)(img + 1024 + i))->EndingLBA - ((gpt_entry_t*)(img + 1024 + i))->StartingLBA + 1) << 9;
                memcpy(&pguid, &((gpt_entry_t*)(img + 1024 + i))->UniquePartitionGUID, sizeof(guid_t));
...
noesp:  if(verbose) printf("BPB bps %u spc %u spf16 %u spf32 %u BPB copy LBA %u\r\n", bpb->bps, bpb->spc, bpb->spf16, bpb->spf32,
    ...
    /* add loaders and a FOSSBIOS Boot Catalog after the GPT header */
    ...
    if(fat_dirent(bpb->rc, 1, "EFI", 1, &clu, 0) && fat_dirent(clu, 1, "BOOT", 1, &clu, 0) &&
      ...
      dev_write(f, (data_lba + clu * bpb->spc) << 9, loader_x86_efi, sizeof(loader_x86_efi))) {
        ...
        fbcat->entries[0].lba = *((uint32_t*)(img + 0x1b0)) = data_lba + clu * bpb->spc;
